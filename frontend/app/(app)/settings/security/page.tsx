...
import { Bot, Clock, KeyRound, Loader2, MapPin, Plus, RefreshCw, ShieldCheck, ShieldOff, Smartphone, Trash2 } from "lucide-react";
...
export default function SecuritySettingsPage() {
  ...
  const [adguardBaseUrl, setAdguardBaseUrl] = useState("");
  ...
  const [savingAdguard, setSavingAdguard] = useState(false);
  const [flushingDns, setFlushingDns] = useState(false);
  const [dnsFlushNotice, setDnsFlushNotice] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/auth/me")
      ...
      .then((me) => {
        ...
        if (admin) loadAdguardConfig();
      })
      .catch(() => {});
  }, []);

  function loadAdguardConfig() {
    fetch("/api/system/dns-flush/config")
      .then((res) => (res.ok ? res.json() : null))
      .then((data) => {
        if (data) {
          setAdguardConfigured(data.configured);
          setAdguardBaseUrl(data.base_url ?? "");
          setAdguardUsername(data.username ?? "");
        }
      })
      .catch(() => {});
  }

  async function handleSaveAdguard(e: React.FormEvent) {
    e.preventDefault();
    setSavingAdguard(true);
    try {
      const res = await fetch("/api/system/dns-flush/config", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          base_url: adguardBaseUrl, username: adguardUsername,
          password: adguardPassword || null,
        }),
      });
      ...
      if (res.ok) {
        setAdguardConfigured(data.configured);
        setAdguardPassword("");
        setNotice(t("security.adguard_saved"));
      }
    } finally {
      setSavingAdguard(false);
    }
  }

  async function handleFlushDns() {
    setFlushingDns(true);
    setDnsFlushNotice(null);
    try {
      const res = await fetch("/api/system/dns-flush", { method: "POST" });
      const data = await res.json();
      setDnsFlushNotice(res.ok ? t("security.dns_flush_success") : (data.detail ?? t("security.dns_flush_error")));
    } catch {
      setDnsFlushNotice(t("security.dns_flush_error"));
    } finally {
      setFlushingDns(false);
      setTimeout(() => setDnsFlushNotice(null), 4000);
    }
  }
  ...
  return (
    <main className="mx-auto w-full max-w-2xl flex-1 overflow-y-auto p-6">
          ...
          {isAdmin && (
            <section className="mt-6 rounded-xl border border-base-border bg-base-elevated p-5 shadow-card">
              <h2 className="flex items-center gap-2 font-display text-base text-ink">
                <RefreshCw className="h-4 w-4" /> {t("security.adguard_heading")}
              </h2>
              <p className="mt-1 text-sm text-ink-muted">{t("security.adguard_description")}</p>

              <form onSubmit={handleSaveAdguard} className="mt-4 space-y-3">
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <label className="block">
                    <span className="mb-1.5 block text-xs font-medium text-ink-muted">{t("security.adguard_base_url_label")}</span>
                    <input
                      type="text"
                      value={adguardBaseUrl}
                      onChange={(e) => setAdguardBaseUrl(e.target.value)}
                      placeholder="http://adguard.local:3000"
                      className="input font-mono text-sm"
                    />
                  </label>
                  <label className="block">
                    <span className="mb-1.5 block text-xs font-medium text-ink-muted">{t("security.adguard_username_label")}</span>
                    <input
                      type="text"
                      value={adguardUsername}
                      onChange={(e) => setAdguardUsername(e.target.value)}
                      className="input"
                    />
                  </label>
                </div>
                <label className="block">
                  <span className="mb-1.5 block text-xs font-medium text-ink-muted">{t("security.adguard_password_label")}</span>
                  <input
                    type="password"
                    value={adguardPassword}
                    onChange={(e) => setAdguardPassword(e.target.value)}
                    placeholder={adguardConfigured ? t("security.adguard_password_unchanged_placeholder") : ""}
                    autoComplete="off"
                    className="input"
                  />
                </label>
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <button type="submit" disabled={savingAdguard} className="submit-button w-auto px-4">
                    {savingAdguard ? <Loader2 className="h-4 w-4 animate-spin" /> : t("common.save")}
                  </button>
                  {adguardConfigured && (
                    <button
                      type="button"
                      ...
                      className="method-button w-auto px-4"
                    >
                      {flushingDns ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}
                      {t("security.dns_flush_button")}
                    </button>
                  )}
                  {dnsFlushNotice && <span className="text-xs text-ink-muted">{dnsFlushNotice}</span>}
                </div>
              </form>
            </section>
          )}
